...
import {updateCart,removeToCart} from "../../lib/redux/reducers/cart";

function Row({id, name, price, quantity}) {
    const dispatch = useDispatch();
    const handleOnChange = (e) => dispatch(updateCart(id, e.target.value))
    const handleOnClick = (e) => dispatch(removeToCart(id))
    return (
        <tr>
            <td data-th="Product" width={{width: "60%"}}>
                <div className="row">
            ...
            <td data-th="Price">${price * quantity} </td>
            <td data-th="Quantity">
                <input type="number" className="form-control form-control-lg text-center"
                       onChange={handleOnChange} value={quantity}/>
            </td>
